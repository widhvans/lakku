            ...
            if caption:
                for channel_id in post_channels:
                    try:
                        ...
                        await client.send_message(user_id, f"⚠️ **Action Required!**\nI could not post to channel `{channel_id}`. Please make me an admin there.")
...
async def new_file_handler(client, message):
    try:
        ...
        owner_db_channel_id = Config.OWNER_DATABASE_CHANNEL
        ...
        try:
            ...
            if "Peer id invalid" in str(e):
                logger.warning("Peer ID not found. Attempting to self-heal by getting chat info.")
                await client.get_chat(owner_db_channel_id) # This "learns" the chat
                copied_message = await message.copy(chat_id=owner_db_channel_id) # Retry the copy
            else:
                raise e # Re-raise other ValueErrors
                
        if copied_message:
            await save_file_data(owner_id=user_id, original_message=message, copied_message=copied_message)
        ...
        await client.send_message(Config.ADMIN_ID, f"⚠️ **CRITICAL ERROR in File Saving**\n\n**Error:** `{e}`\n\nPlease ensure the `OWNER_DATABASE_CHANNEL` ID is correct and I am an admin there.")
